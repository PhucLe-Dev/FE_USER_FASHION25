...
export default function RelatedProductCart() {
   // Khai báo state để lưu danh sách loại sản phẩm
    ...
    useEffect(() => {
        ...
        const fetchDataSanPham = async () => {
            try {
                const response = await fetch("http://localhost:3000/api/user/san-pham?random=true&limit=8", {
                    method: "GET",
                    headers: {
                    ...
                    <Link href={`/detail-product/${item.variants[0]?.sku}`} key={item._id} className="product-card bg-white group relative">
                        <div className="relative overflow-hidden">
                            ...
                            <div
                                ...
                                <button
                                    className="cursor-pointer bg-white text-primary px-8 py-3 opacity-0 group-hover:opacity-100 transition duration-300 transform translate-y-4 group-hover:translate-y-0 hover:bg-[#ebbd5b] hover:text-white font-medium tracking-wider">CHI TIẾT</button>
                            </div>

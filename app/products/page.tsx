...
export default function ProductsPage({ slug }: Props) {
    const [showSidebar, setShowSidebar] = useState(false);

    const [listProduct, setListProduct] = useState<ISanPham[]>([]);
    ...
    // Thêm các state để khắc phục lỗi
    ...
    const fetchFilters = useCallback(async () => {
        try {
            const [categoriesResponse, brandsResponse, originsResponse] = await Promise.all([
                fetch('http://localhost:3000/api/user/loai-san-pham?limit=100'),
                fetch('http://localhost:3000/api/user/thuong-hieu?limit=100'),
                fetch('http://localhost:3000/api/user/xuat-xu'),
            ]);
    ...
    // Activate filter from slug
    useEffect(() => {
        const activateFilterFromSlug = async () => {
            setCategoryFilter([]);
            setBrandFilter([]);

            if (!slug || (categories.length === 0 && brands.length === 0)) return;

            try {
                const loaiResponse = await fetch(`http://localhost:3000/api/user/loai-san-pham?slug=${slug}`);
                const thuongHieuResponse = await fetch(`http://localhost:3000/api/user/thuong-hieu?slug=${slug}`);

                const loaiData = await loaiResponse.json();
                const thuongHieuData = await thuongHieuResponse.json();

                if (loaiData.data && loaiData.data.length > 0) {
                    const categoryName = loaiData.data[0].ten_loai;
                    if (categoryName && categories.includes(categoryName)) {
                        setCategoryFilter(prev => [...prev, categoryName]); // Thêm vào mảng hiện tại
                    }
                } else if (thuongHieuData.data && thuongHieuData.data.length > 0) {
                    const brandName = thuongHieuData.data[0].ten_thuong_hieu;
                    if (brandName && brands.includes(brandName)) {
                        setBrandFilter(prev => [...prev, brandName]); // Thêm vào mảng hiện tại
                    }
                }
            } catch (error) {
                console.error('Lỗi khi active filter từ slug:', error);
            }
        };

        if (categories.length > 0 && brands.length > 0) {
            activateFilterFromSlug();
        }
    }, [slug, categories, brands]);
    ...
    // Fetch products
    const fetchDataSanPham = useCallback(async () => {
        setIsLoading(true);
        try {
            ...
            // Luôn gửi tất cả bộ lọc, kể cả khi có slug
            ...
            if (categoryFilter.length > 0) {
                categoryFilter.forEach(category => {
                    if (categoryMap[category]) {
                        params.append('category', categoryMap[category].toString());
                    }
                });
            }
            if (brandFilter.length > 0) {
                brandFilter.forEach(brand => {
                    if (brandMap[brand]) {
                        params.append('brand', brandMap[brand].toString());
                    }
                });
            }
            if (originFilter.length > 0) {
                originFilter.forEach(origin => {
                    if (originMap[origin]) {
                        params.append('origin', originMap[origin]);
                    }
                });
            }

            let endpoint = '/api/user/san-pham'; // Default endpoint
            if (slug) {
                endpoint = `/api/user/san-pham/slug/${slug}`;
            }
            const fullEndpoint = `${endpoint}?${params.toString()}`;
            console.log('API Call:', fullEndpoint);

            const response = await fetch(`http://localhost:3000${fullEndpoint}`, {
                method: 'GET',
    ...
    }, [slug, pagination.currentPage, sortType, categoryFilter, originFilter, brandFilter, categoryMap, brandMap, originMap]);
    ...
    const handleCategoryChange = (category: string) => {
        setCategoryFilter(prev => {
            const newFilters = prev.includes(category)
                ...
                : [...prev, category]; // Cho phép tích nhiều loại
            return newFilters;
        });
        setPagination(prev => ({ ...prev, currentPage: 1 }));
        closeSidebar();
    ...
    const handleOriginChange = (origin: string) => {
        setOriginFilter(prev => {
            const newFilters = prev.includes(origin)
                ? prev.filter(item => item !== origin)
                : [...prev, origin]; // Cho phép tích nhiều xuất xứ
            return newFilters;
        });
        setPagination(prev => ({ ...prev, currentPage: 1 }));
        closeSidebar();
    ...
    const handleBrandChange = (brand: string) => {
        setBrandFilter(prev => {
            const newFilters = prev.includes(brand)
                ? prev.filter(item => item !== brand)
                : [...prev, brand]; // Cho phép tích nhiều thương hiệu
            return newFilters;
        });
        setPagination(prev => ({ ...prev, currentPage: 1 }));
        closeSidebar();
    ...
    return (
        <section className="pt-15 pb-20 bg-[#f8f8f8]">
            ...
            <div className="container mx-auto px-6 my-[-30px]">
